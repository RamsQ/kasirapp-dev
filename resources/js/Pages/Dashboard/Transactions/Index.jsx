        ...
        <div className="flex flex-col gap-1 mb-3">
            <div className={`flex flex-col p-3 border rounded-2xl shadow-sm group transition-all ${isBonus ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-slate-800/40 dark:border-slate-700'}`}>
                <div className="flex items-center gap-2">
                    ...
                    {!isBonus && (
                        <>
                            <button onClick={() => setShowNoteInput(!showNoteInput)} className={`p-1 rounded-md transition-colors ${c.notes ? 'text-primary-500 bg-primary-50 dark:bg-primary-950/30' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}><IconInfoCircle size={14} /></button>
                    ...
                    <button onClick={() => deleteCart(c.id)} className="text-slate-300 hover:text-red-500 transition-colors ml-1"><IconX size={14} /></button>
                </div>
                ...
                {showNoteInput && (
                    <div className="mt-2 flex gap-1 animate-in slide-in-from-top-1 duration-200">
                        ...
                        <button onClick={handleSaveNote} className="bg-primary-500 text-white p-1.5 rounded-lg active:scale-90 transition-transform"><IconCheck size={14} /></button>
                    </div>
                )}
            </div>
        </div>
...
const Index = ({ carts = [], products: initialProducts, customers = [], discounts = [], paymentSetting = {}, activeShift = null, holds = [], tables = [], categories = [], filters = {}, onlineSettings = [] }) => {
    ...
    useEffect(() => {
        if (window.Echo) {
            window.Echo.channel('public-order.1').listen('.order.placed', () => {
                router.reload({ only: ['holds'], preserveScroll: true });
                toast.success("Antrean Diperbarui!", { icon: '🔔' });
            });
            return () => { window.Echo.leave('public-order.1'); };
        }
    }, []);
    ...
    const getFinalPrice = useCallback((product) => {
        ...
        if (selectedPlatform) {
            ...
            price = price + markup + parseFloat(selectedPlatform.additional_fee);
        }
    ...
    // --- LOGIKA PERHITUNGAN TOTAL & DISKON GLOBAL ---
    ...
    const addToCart = (product) => {
        ...
        router.post(route("transactions.addToCart"), { product_id: product.id }, { preserveScroll: true });
    };
    ...
    const submitTransaction = (method, paidAmount) => {
        ...
        router.post(route("transactions.store"), {
            ...
            grand_total: grandTotal, cash: paidAmount, change: change, payment_gateway: method, 
            hold_id: finalHoldId, queue_number: queueNumber, 
        ...
        }, { onSuccess: () => { setCash(0); setActiveHoldId(null); setSelectedTable(""); setShowCartDrawer(false); } });
    };

    // --- FUNGSI SAVE ORDER (PENTING: FIX UNCAUGHT ERROR) ---
            ...
            {!activeShift && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 text-center">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-300">
                        ...
                        <button onClick={(e) => { e.preventDefault(); postShift(route('shifts.store')); }} className="w-full py-4 mt-6 bg-primary-600 text-white rounded-2xl font-black uppercase shadow-lg active:scale-95 transition-all">Mulai Bertugas</button>
                    </div>
                </div>
            )}
            ...
            <div id="main-app-content" className={`flex flex-col h-[100dvh] w-full transition-colors ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-100 text-slate-800'} overflow-hidden print:hidden ${!activeShift ? 'blur-xl pointer-events-none' : ''}`}>
                <header className="h-16 bg-white dark:bg-slate-900 border-b dark:border-slate-800 flex items-center justify-between px-4 md:px-6 shrink-0 z-20 shadow-sm">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        <button onClick={() => setShowCashOut(true)} className="flex px-4 py-2 bg-orange-100 text-orange-600 dark:bg-orange-950/30 dark:text-orange-400 rounded-xl text-[10px] font-black uppercase items-center gap-2 border border-orange-200 shadow-sm hover:bg-orange-200 transition-colors"><IconCashOff size={16}/> <span className="hidden sm:inline">Kas Keluar</span></button>
                        ...
                        <button onClick={toggleTheme} className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 border dark:border-slate-700">{isDarkMode ? <IconSun size={20} className="text-yellow-400" /> : <IconMoon size={20} />}</button>
                        <button onClick={() => carts.length > 0 ? Swal.fire("Peringatan", "Kosongkan keranjang dulu!", "warning") : setShowCloseShift(true)} className="p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 text-red-500 border border-red-100"><IconPower size={20} /></button>
                    </div>
                </header>
                ...
                <main className="flex flex-1 overflow-hidden lg:flex-row flex-col relative">
                    <div className="flex-1 flex flex-col min-w-0">
                        ...
                        <div className="bg-white/30 dark:bg-slate-900/30 border-b dark:border-slate-800 p-2 flex items-center gap-2 overflow-x-auto scrollbar-hide shrink-0">
                            <button onClick={() => handleCategoryChange('all')} className={`px-5 py-2 rounded-2xl font-black uppercase text-[10px] border transition-all ${selectedCategory === 'all' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>Semua</button>
                            {categories.map((cat) => (
                                <button key={cat.id} onClick={() => handleCategoryChange(cat.id.toString())} className={`px-5 py-2 rounded-2xl font-black uppercase text-[10px] border transition-all ${selectedCategory === cat.id.toString() ? 'bg-primary-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>{cat.name}</button>
                            ))}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                            <div className={viewMode === 'grid' ? "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4" : "flex flex-col gap-2"}>
                                {productList.map((p) => {
                                    ...
                                    return (
                                        <button key={p.id} onClick={() => addToCart(p)} className={`relative text-left group border dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm rounded-[1.5rem] p-3 transition-all active:scale-95 hover:shadow-xl ${viewMode === 'list' ? 'flex items-center gap-4' : 'flex flex-col'}`}>
                        ...
                        <div className={`flex flex-col flex-1 overflow-hidden transition-opacity duration-300 ${!showCartDrawer && window.innerWidth < 1024 ? 'opacity-0 invisible' : 'opacity-100 visible'}`}>
                            ...
                            <div className="p-4 md:p-6 bg-white dark:bg-slate-950 border-t dark:border-slate-800 space-y-4 shadow-xl shrink-0">
                                ...
                                <div className="flex justify-between items-end">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Total Bayar</span>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button onClick={() => submitTransaction('qris', grandTotal)} disabled={carts.length === 0} className="py-4 bg-primary-600 text-white font-black text-[10px] rounded-2xl uppercase active:scale-95 disabled:opacity-50 transition-all shadow-lg hover:bg-primary-700 flex items-center justify-center gap-2"><IconQrcode size={18} /> QRIS</button>
                                </div>
            ...
            {showModalHold && (
                <div className="fixed inset-0 z-[150] flex flex-col bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="h-20 bg-white dark:bg-slate-900 border-b dark:border-slate-800 flex items-center justify-between px-6 md:px-10 shrink-0 shadow-sm">
                        ...
                        <button onClick={() => setShowModalHold(false)} className="p-3 bg-red-50 dark:bg-red-950/30 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"> <IconX size={24} /> </button>
                    </div>
                                ...
                                <div className="p-6 flex flex-col h-full">
                                    ...
                                    <div className="flex-1 space-y-3 mb-6 bg-slate-50 dark:bg-slate-800/40 p-4 rounded-[1.5rem] border border-slate-100 dark:border-slate-700/50 overflow-y-auto max-h-[180px] custom-scrollbar shadow-inner">
                                        {(h.cart_data || []).map((item, idx) => (
                                            <div key={idx} className="flex flex-col border-b border-slate-200/50 dark:border-slate-700/50 last:border-none pb-2 mb-2 last:pb-0 last:mb-0">
                                                ...
                                                {item.product?.type === 'bundle' && item.product?.bundle_items?.map((bi, bIdx) => (
                                                    <div key={bIdx} className="text-[8px] font-bold text-slate-400 uppercase italic ml-4 mt-1 flex items-center gap-1">
                                                        <IconArrowsExchange size={10} className="text-slate-300" />
                                                        {bi.title} <span className="text-slate-500">({parseFloat(bi.pivot?.qty || 1) * item.qty})</span>
                                                    </div>
                                                ))}
                                            </div>
                                        ))}
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => {
                                            ...
                                            toast.promise(printBillUsb(billData, receiptSetting), { loading: 'Mencetak Bill...', success: 'Bill dicetak!', error: 'Gagal cetak' });
                                        ...
                                        <button onClick={() => handleResumeHold(h.id)} className="flex-1 py-4 bg-primary-600 text-white rounded-2xl font-black uppercase text-[11px] shadow-lg shadow-primary-500/20 active:scale-95 hover:bg-primary-700 transition-all flex items-center justify-center gap-2">
                                            <IconCash size={18} /> Bayar
                                        </button>
                                        <button onClick={() => { Swal.fire({ title: 'Hapus Pesanan?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444' }).then(res => { if(res.isConfirmed) router.delete(route('holds.destroy', h.id)) }) }} className="p-4 bg-red-50 dark:bg-red-950/30 text-red-600 rounded-2xl hover:bg-red-500 hover:text-white transition-all active:scale-90"> <IconTrash size={20}/> </button>
                                    </div>
                                </div>
                            </div>
                        ))}
                         ...
                         <div className="space-y-4">
                            ...
                            <div className="flex gap-3 mt-4">
                                ...
                                <button 
                                    ...
                                    disabled={isCashOutLoading || !cashOutName || !cashOutAmount} 
                                    className="flex-[2] py-4 bg-orange-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg transition-all active:scale-95 disabled:opacity-50"
                                >
